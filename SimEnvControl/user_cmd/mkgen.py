...
from ..libsimenv.checkpoints_db import check_checkpoint_exist
...
def mkgen_bootstrap(manifest, repo_path):
    ...
    actual_app_cmd = add_base_to_stdin_file_in_shcmd(manifest["app_init_cwd"], "$(SIMENV_SYSROOT)", "$(APP_INIT_CWD)")
...
def cmd_mkgen(ctx, app_name, checkpoint, bootstrap):
    ...
    try:
        ...
        if bootstrap:
            ...
            generated_makefile = mkgen_normal(manifest, checkpoint)
